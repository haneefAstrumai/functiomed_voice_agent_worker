# """
# agent/main.py — Functiomed Voice Agent (Single LLM)
# """
...
#         self.step:         str        = "idle"  # idle→service→doctor→slot→name→done
#         self._services:    list       = []
#         self._doctors:     list       = []
#         self._slots:       list       = []
...
#     ) -> None:
#         """
#         Fires after every user message, BEFORE the LLM generates a reply.
#         Injects retrieved clinic knowledge into chat context.
#         Skipped during active booking steps.
#         """
#         user_text = new_message.text_content or ""

#         # Skip RAG once booking has started
...
#         # Store for exact name resolution in confirm_service
...
#         # Match against stored list to get exact casing from DB
...
#     @function_tool()
#     async def get_doctors(self, context: RunContext) -> str:
...
#         # Store raw doctors for exact name resolution in confirm_doctor
...
#         # Fuzzy match against stored list
#         matched = next(
#             (d for d in stored
...
#         if matched:
#             # Reconstruct full display name with title from DB
...
#     @function_tool()
#     async def get_slots(self, context: RunContext) -> str:
...
#         # Store raw slots for UUID resolution in confirm_slot
#         self._booking._slots = slots

#         lines = []
#         for i, s in enumerate(slots[:5], 1):
#             lines.append(
#                 f"{i}. {s['slot_date']} at {s['slot_time']} — slot_id: {s['id']}"
#             )
...
#         slot_id must be the UUID string from get_slots (e.g. 'abc-123-uuid').
#         Never pass a number like '1' or '2'.
#         """
...
#         # Fallback: match by date + time if LLM passed a number instead of UUID
#         if not matched:
#             matched = next(
...
#             )
#             return (
#                 f"I couldn't find that slot. Please choose from:\n{slots_available}"
#             )
...
#         # Handle both response shapes: {"booking": {...}} or flat {...}
#         booking = result.get("booking") or result
#         conf    = booking.get("confirmation_number")

#         if conf:
#             self._booking = BookingState()  # reset state
#             return (
...
#         llm=openai.LLM(model=os.getenv("AGENT_LLM_MODEL", "gpt-4o-mini")),
#     #     llm=GroqLLM(
